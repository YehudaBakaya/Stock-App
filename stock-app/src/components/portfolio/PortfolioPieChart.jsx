        ...
        <div className="bg-gray-900/95 backdrop-blur-sm border border-gray-700 rounded-xl p-4 shadow-2xl">
          ...
          <div className="space-y-1 text-sm">
            <p className="text-gray-300">
              <span className="text-gray-500">שווי:</span> ${data.value.toLocaleString()}
            </p>
            <p className="text-gray-300">
              <span className="text-gray-500">מניות:</span> {data.shares}
            </p>
            <p className="text-gray-300">
              <span className="text-gray-500">מחיר נוכחי:</span> ${data.currentPrice.toFixed(2)}
            </p>
            <p className={isPositive ? 'text-green-400' : 'text-red-400'}>
              <span className="text-gray-500">רווח/הפסד:</span> {isPositive ? '+' : ''}{data.pnl}%
            </p>
          </div>
        </div>
      );
    ...
    return (
      <Card>
        <div className="p-12 text-center">
          ...
          <p className="text-gray-400 text-lg">אין מניות בתיק</p>
          <p className="text-gray-500 text-sm mt-2">הוסף מניות כדי לראות את הפיזור שלך</p>
        </div>
      </Card>
    );
  }
    ...
    <Card>
      <div className="border-b border-gray-700/50 pb-4 mb-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-left">
            <p className="text-gray-400 text-sm">שווי כולל</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-6">
        {portfolioData.map((item, index) => (
          <div key={item.name} className="flex items-center gap-2 bg-gray-800/50 rounded-lg p-2">
            <div 
